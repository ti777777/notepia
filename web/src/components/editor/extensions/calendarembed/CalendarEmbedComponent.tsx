...
import { Calendar, ChevronUp, ChevronDown, ExternalLink, Trash2 } from "lucide-react"
import { useQuery } from "@tanstack/react-query"
import { useNavigate } from "react-router-dom"
import { getViewObjects } from "@/api/view"
import useCurrentWorkspaceId from "@/hooks/use-currentworkspace-id"
import CalendarViewComponent from "@/components/views/calendar/CalendarViewComponent"
import { TwoColumnContext } from "@/components/twocolumn/TwoColumn"
import ViewBlockPicker from "../viewblockpicker/ViewBlockPicker"

const mockTwoColumnCtx = {
...
const CalendarEmbedComponent: React.FC<NodeViewProps> = ({ node, updateAttributes, selected, editor, deleteNode, getPos }) => {
  ...
  const { data: viewObjects } = useQuery({
    queryKey: ['view-objects', workspaceId, viewId],
    queryFn: () => getViewObjects(workspaceId, viewId!),
  ...
  return (
    <NodeViewWrapper>
      ...
      >
        <TwoColumnContext.Provider value={mockTwoColumnCtx}>
        ...
        {isEditable && (showActions || selected) && (
          <div className="absolute top-2 right-2 flex gap-1 z-50">
            ...
            <button onClick={() => navigate(`/workspaces/${workspaceId}/calendar/${viewId}`)} className="p-2 bg-white dark:bg-neutral-800 hover:bg-gray-100 dark:hover:bg-neutral-700 rounded-lg shadow-lg border border-gray-200 dark:border-neutral-600 transition-colors" title="Open full view">
...
export default CalendarEmbedComponent
